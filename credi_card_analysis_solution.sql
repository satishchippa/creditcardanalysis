-- Queries to analyse the credit card transactions data.
...
select min(transaction_date) as min, max(transaction_date) as max from credit_card_transactions;
...
-- when it reaches a cumulative of  1,000,000 total spends(We should have 4 rows in the o/p one for each card type)
...
--Q8 Which city has highest total spend to total no of transactions ratio during weekends
